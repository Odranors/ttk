...
def send(did, iid, cdid, openudid):
    ...
    for x in range(10):
        try:
            ...
            try:
                _lock.acquire()
                print(Colorate.Horizontal(Colors.red_to_white, f"+ - sent views {response.json()['log_pb']['impr_id']} {__aweme_id} {reqs}"))
                _lock.release()
                _lock.release()
                success += 1
            except:
...
def title_loop():
    ...
    if os.name == "nt":
        while True:
            os.system(f'title TikTok Viewbot by @Rodin.ff ^| success: {success} fails: {fails} reqs: {reqs} rps: {rps} rpm: {rpm}')
            time.sleep(0.1)

if __name__ == "__main__":
    os.system("cls" if os.name == "nt" else "clear"); os.system("title TikTok Viewbot by @Rodin.ff" if os.name == "nt" else "")
    
    txt = """     _  __    _   ______  ____  _________    __   _____    ____  _______    __\n    | |/ /   / | / / __ \/ __ \/ ____/   |  / /  / ___/   / __ \/ ____/ |  / /\n    |   /   /  |/ / / / / /_/ / __/ / /| | / /   \__ \   / / / / __/  | | / /\n   /   |   / /|  / /_/ / _, _/ /___/ ___ |/ /______/ /  / /_/ / /___  | |/ /\n  /_/|_|  /_/ |_/\____/_/ |_/_____/_/  |_/_____/____/  /_____/_____/  |___/\n                             By : @Rodin.ff"""
    ...
    print(
        Colorate.Vertical(
            Colors.DynamicMIX((Col.red, Col.red)), Center.XCenter(txt),
        )
   ) 
    
    try:
        link = str(Write.Input("\n\n            [+] Video Link > ", Colors.white, interval=0.0001))
        __aweme_id = str(
